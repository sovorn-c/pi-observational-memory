...
7. If the summary is empty, return no extension result so Pi uses native compaction. Otherwise return `{ compaction: { summary, firstKeptEntryId, tokensBefore, details } }` where `details.type` is `om.folded`.

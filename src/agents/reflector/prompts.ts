...
These records are the ONLY information the assistant will have about past interactions once the raw conversation is compacted out of context. Anything you fail to preserve may be forgotten. Anything you distort may be remembered wrong. Take this seriously.

Your task is different from the observer's: you are not recording events, you are distilling stable, long-lived facts and patterns from active observations into new reflections by calling record_reflections.
...
- Prefer high and critical observations, then older medium observations whose durable meaning can be preserved with equivalent fidelity.
...
- Most transient task-log observations, tool status, one-off attempts, and partial work should not become reflections. Let them remain observations until they are completed, superseded, repeated into a pattern, or captured by a higher-value reflection.
...
- BAD: completed: edited src/auth/middleware.ts.
- GOOD: completed: auth middleware now validates JWTs in src/auth/middleware.ts and the user confirmed tests pass.
